"""
Older version of the script in this script only inserts were implemented
and last_id is being treated as single value, uploaded this to have back up
- Vyshnavi
...
class MigrationScheduler:
    ...
    def get_migration_details(self, job_name, conn):
        try:
            query = f"SELECT * FROM migrations WHERE migration_name = '{job_name}'"
    ...
    def main_migration_func(self,job_name,postgres_conn):
        ...
        migration_details_dict=self.get_migration_details(job_name,postgres_conn)
        ...
        if table_flag:
            ...
            if last_id==0 and migration_status=='first':
                ...
                df=self.get_all_records(from_connection,full_from_table)
                ...
                print(f"Details to update in table {migration_details_dict_updated}")
                ...
                migrate_records=f"select * from {full_from_table} where id > {last_id}"
                print(f"records query : {migrate_records}")
    ...
    def get_all_records(self,connection,table_name):
        try:
            df_rows=f"select * from {table_name}"
    ...
    def insert_records_postgresql_batch_5(self, postgres_connection, df, insert_table_name, migration_track_col):
        ...
        columns=self.enclose_uppercase_words(columns)
        ...
        try:
            ...
            for batch_index in range(num_batches):
                ...
                # insert_query = f"INSERT INTO {insert_table_name} ({columns}) VALUES ({placeholders})"
